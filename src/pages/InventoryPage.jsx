...
import { inventoryItems } from '../data/placeholder';
...
export default function InventoryPage({ isMobile }) {
  ...
  const [addForm, setAddForm] = useState({ name:'',unit:'kg',stock:0,minStock:0,cost:0,category:'',supplier:'' });
  // Wastage form
  ...
  const handleAddStock = () => {
    setItems(prev => [...prev, { id: Date.now(), ...addForm, stock: Number(addForm.stock), minStock: Number(addForm.minStock), cost: Number(addForm.cost) }]);
    setAddForm({ name:'',unit:'kg',stock:0,minStock:0,cost:0,category:'',supplier:'' });
    setShowAdd(false);
  };

  const handleWastage = () => {
    const qty = Number(wastageForm.qty);
    setItems(prev => prev.map(i => i.id===Number(wastageForm.itemId) ? { ...i, stock: Math.max(0, i.stock-qty) } : i));
